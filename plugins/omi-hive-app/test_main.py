...
class HiveAppTests(unittest.TestCase):
    ...
    def test_create_task_success(self):
        ...
        with patch.object(app_module, "is_connected", return_value=True):
            # find_project_by_name returns (project, candidates); patching it with a
            # bare project made the handler's tuple unpack raise, so the assertion below
            # was reading result off an error response.
