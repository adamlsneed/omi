...
def validate(root: Path = ROOT) -> list[str]:
    ...
    if (root / AUTO_WORKFLOW_PATH).is_file():
        missing = [
            str(path)
            for path in (ADMISSION_VERIFIER_PATH, AUTO_ADMISSION_VERIFIER_PATH, DEPLOY_BACKEND_STACK_ACTION)
            if not (root / path).is_file()
        ]
        if missing:
            return [f"backend source-admission contract is missing: {path}" for path in missing]
        errors.extend(validate_auto_workflow((root / AUTO_WORKFLOW_PATH).read_text(encoding="utf-8"), root))
