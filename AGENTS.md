...
- **Hosted Backend Config** — local/dev builds must default to BasedHardware/Omi hosted backend services when testing against a paid Omi subscription. Do not commit personal custom API endpoints, private backend URLs, or user-specific API credentials into the app; keep those in ignored local env files or runtime configuration. When testing hosted auth locally, prefer the Omi auth exchange (`API_BASE_URL=https://api.omi.me/`, `USE_WEB_AUTH=true`, `USE_AUTH_CUSTOM_TOKEN=true`) over direct dev Firebase tokens unless explicitly testing a local backend.
- **Hosted-auth Firebase project must match the backend** — `https://api.omi.me/` mints Firebase custom tokens for the **`based-hardware`** project (project `208440318997`). There is no `based-hardware-prod`; `based-hardware-dev` (`1031333818730`) is a *different* project. A local build must initialize **`based-hardware`** Firebase or sign-in fails right after the hosted browser redirect with `[firebase_auth/custom-token-mismatch] The custom token corresponds to a different audience.` (the `/v1/auth/token` exchange still returns 200 — only the client `signInWithCustomToken` fails). For Adam's local iPhone build this means the **dev flavor** (for its working `.dev.env` hosted/web-auth config) with `app/lib/firebase_options_dev.dart` and the `ios/.../GoogleService-Info.plist` files repointed at `based-hardware` — **not** the prod flavor (its `.prod.env` is absent locally → `apiBaseUrl=null` + native Google Sign-In that crashes on the re-bundled id). The `based-hardware` client config is in `desktop/macos/Desktop/Sources/GoogleService-Info.plist`. Full diagnosis + the on-device log-capture recipe (profile builds emit no Dart logs to `idevicesyslog`/`devicectl console`/`flutter attach`): `app/docs/local-ios-standalone-install.md`.
- **Always bump the build number before a local device / hardware / desktop build you install or flash** (Adam's standing preference — every installed artifact must be uniquely identifiable; reusing a number across two different binaries on the same device is the ambiguity this prevents). Do the bump as part of the build step, not after:
  - **iPhone app (Flutter):** increment the `+N` build in `app/pubspec.yaml` (`version: X.Y.Z+N`), then **re-run `flutter build ios --config-only`** (or `flutter run`) so the bump propagates into the iOS `CFBundleVersion`. A bare `xcodebuild` against a stale `ios/Flutter/Generated.xcconfig` keeps the OLD `FLUTTER_BUILD_NUMBER`, so the installed app's build number won't change — verify with `PlistBuddy -c 'Print :CFBundleVersion' <built>.app/Info.plist` before installing.
  - **macOS desktop app:** increment `CFBundleVersion` in `desktop/macos/Desktop/Info.plist`.
  - **omi device firmware (CV01 = `omi/firmware/omi/`):** bump `CONFIG_BT_DIS_FW_REV_STR` in `omi/firmware/omi/omi.conf` (model `Omi CV 1`; see that file for the current value) — that string is the firmware revision the device advertises over the BLE Device Information Service and the app displays. The DevKit hardware uses its own `omi/firmware/devkit/prj_*.conf`. For OTA/MCUboot updates, also bump the signed image version at sign time via imgtool `--version` (not pinned in-repo).
