...
- **Always bump the build number before a local device / hardware / desktop build you install or flash** (Adam's standing preference — every installed artifact must be uniquely identifiable; reusing a number across two different binaries on the same device is the ambiguity this prevents). Do the bump as part of the build step, not after:
  ...
  - **omi device firmware:** bump its version/build identifier before flashing. (No build-number field is exposed in-repo today — `omi/firmware/omi/CMakeLists.txt` `project(omi)` carries no version — so confirm the exact field/mechanism with Adam at the first firmware flash.)
