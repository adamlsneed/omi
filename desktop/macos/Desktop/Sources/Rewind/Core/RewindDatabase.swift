...
actor RewindDatabase {
  ...
  private func performInitializationWithRetry(
    expectedUserId: String,
    expectedGeneration: Int
  ) async throws {
    var lastError: Error?
    // Our own between-attempt close() bumps initGeneration, so each attempt
    // re-captures the current generation. Retries stop if the target user
    // changed (an external close/retarget must not be resurrected here).
    var attemptGeneration = expectedGeneration
    ...
    for attempt in 0..<maxInitRetries {
      do {
        ...
        guard targetUserId() == expectedUserId else { break }
        attemptGeneration = initGeneration
    ...
    let dumpSuccess = await withCheckedContinuation { (continuation: CheckedContinuation<Bool, Never>) in
      Task.detached {
        guard FileManager.default.createFile(atPath: dumpPath, contents: nil),
          let dumpHandle = FileHandle(forWritingAtPath: dumpPath)
        else {
  ...
  func updateOCRResult(id: Int64, ocrResult: OCRResult) throws {
    ...
    try dbQueue.write { db in
      try db.execute(
        sql:
          "UPDATE screenshots SET ocrText = ?, ocrDataJson = ?, isIndexed = 1, skippedForBattery = 0, textSource = ? WHERE id = ?",
        arguments: [
          ocrResult.fullText,
          ocrDataJson,
