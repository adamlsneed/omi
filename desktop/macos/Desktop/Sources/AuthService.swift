...
class AuthService {
    ...
    private func restoreAuthState(attempt: AuthSessionAttempt) async {
        ...
        if savedSignedIn {
            ...
            // Guard Auth.auth() behind FirebaseApp.app(): calling it before
            // configure() traps (ad-hoc signed dev builds can hit this path).
            AuthState.shared.userEmail = (FirebaseApp.app() == nil ? nil : Auth.auth().currentUser?.email) ?? savedEmail
    ...
    func signOut() async throws {
        ...
        TrialBannerService.shared.reset()
