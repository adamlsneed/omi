...
class AuthService {
    ...
    -> TokenExchangeResult
  {
    ...
    if let idToken = json["id_token"] as? String {
      if let userInfo = decodeJWT(idToken) {
        ...
        NSLog(
          "OMI AUTH: Extracted from id_token - hasName: %@, hasEmail: %@",
  ...
  private func migrateLegacyForkKeychainTokens() -> StoredAuthTokens? {
    ...
    guard
      ...
      let refreshToken = DesktopKeychainStore.string(
        service: legacyService, account: DefaultsKey.authRefreshToken.rawValue),
      !idToken.isEmpty,
      !refreshToken.isEmpty
    else {
      return nil
