...
struct ShellStatusIcons: View {
  ...
  var body: some View {
    HStack(spacing: 2) {
      ...
      ShellStatusIconButton(
        systemImage: ideaCaptureAction.systemImage,
        tooltip: ideaCaptureAction.helpText,
        state: appState.isIdeaCaptureActive ? .active : .inactive,
        action: toggleIdeaCapture
      )
      .accessibilityIdentifier("shell-status-capture-idea")
    }
  ...
  private var ideaCaptureAction: IdeaCaptureSidebarAction {
    IdeaCaptureSidebarAction.current(isActive: appState.isIdeaCaptureActive)
  }
  ...
  private func toggleIdeaCapture() {
