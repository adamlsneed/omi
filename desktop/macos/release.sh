...
# Mac's login keychain (generated by Sparkle's generate_keys); sign_update reads it.
...
SPARKLE_SIG_LINE="$("$SPARKLE_BIN/sign_update" "$ZIP_PATH")"
...
[ -n "$SPARKLE_SIGNATURE" ] && [ -n "$SPARKLE_LENGTH" ] || { echo "ERROR: sign_update returned no signature: $SPARKLE_SIG_LINE" >&2; exit 1; }
