...
# --- Toolchain: Node on PATH, and npm 10 (npm 12 fails `npm ci` on the committed lockfiles) ---
# Set up here so unattended runs need no PATH prefix. The npm 10 wrapper lives in
# /tmp/npm10 (wiped on reboot) and is recreated from the npx cache when missing.
ensure_toolchain() {
  [ -x "$HOME/.hermes/node/bin/node" ] && PATH="$HOME/.hermes/node/bin:$PATH"
  command -v node >/dev/null 2>&1 || { echo "ERROR: node not found on PATH or in ~/.hermes/node/bin" >&2; exit 1; }
  ...
  PATH="$npm10_dir:$PATH"
  export PATH
  case "$(npm -v)" in 10.*) ;; *) echo "ERROR: npm 10 required, got $(npm -v)" >&2; exit 1;; esac
}
ensure_toolchain
...
# Sparkle feed for the fork bundle: appcast.xml lives next to the cask in the tap repo.
# The public key pairs with the "Private key for signing Sparkle updates" item in this
# Mac's login keychain (generated by Sparkle's generate_keys); /usr/bin/security reads it.
FORK_FEED_URL="${FORK_FEED_URL:-https://raw.githubusercontent.com/${TAP_REPO}/main/appcast.xml}"
FORK_SPARKLE_PUBLIC_KEY="${FORK_SPARKLE_PUBLIC_KEY:-F8Be+CbJ+xo+QdqkLH1jFFIdDIWLZGdFZnzvKa7/zXI=}"
...
# Point Sparkle at the fork feed and key, and mark the bundle so the app applies the
# managed auto-update policy despite the non-production bundle id.
pb "Set :SUFeedURL $FORK_FEED_URL"
pb "Set :SUPublicEDKey $FORK_SPARKLE_PUBLIC_KEY"
pb "Add :OmiForkRelease bool true"
...
# Sign-in goes through BasedHardware's hosted Firebase project; a build without the key
# ships a broken sign-in that Sparkle would auto-install, so a missing key is fatal.
if ! grep -q "^FIREBASE_API_KEY=." "$ENV_OUT"; then
  KEY="${FIREBASE_API_KEY:-}"
  [ -z "$KEY" ] && [ -f "$INSTALLED_ENV" ] && KEY=$(grep "^FIREBASE_API_KEY=" "$INSTALLED_ENV" | head -1 | cut -d= -f2-)
  [ -n "$KEY" ] || { echo "ERROR: FIREBASE_API_KEY not found in .env.app*, the environment, or $INSTALLED_ENV" >&2; exit 1; }
  set_env FIREBASE_API_KEY "$KEY"
fi
...
if [ "${SKIP_NOTARIZE:-0}" = "1" ]; then
  ...
  NOTARY_JSON="$(xcrun notarytool submit "$ZIP_PATH" --keychain-profile "$NOTARY_PROFILE" \
    --wait --timeout 90m --output-format json)" || true
  NOTARY_STATUS="$(printf '%s' "$NOTARY_JSON" | plutil -extract status raw -o - - 2>/dev/null || true)"
  case "$NOTARY_STATUS" in
    Accepted) ;;
    "In Progress")
      echo "ERROR: notarization still in progress after 90 minutes; not publishing. $NOTARY_JSON" >&2
      exit 1;;
    *)
      echo "ERROR: notarization failed (status: ${NOTARY_STATUS:-none}). $NOTARY_JSON" >&2
      exit 1;;
  esac
...
step "Signing $ZIP_PATH for Sparkle"
[ -x "$SPARKLE_BIN/sign_update" ] || { echo "ERROR: $SPARKLE_BIN/sign_update missing (swift build should have fetched Sparkle)" >&2; exit 1; }
# The key is piped from /usr/bin/security rather than read by sign_update itself: keychain
# access is granted per binary path, and every worktree build puts sign_update at a new
# path. Never echo sign_update's output on failure: a bad key read makes it print the key.
# sign_update reads a file literally named "-" in the cwd instead of stdin, so refuse one.
[ ! -e - ] || { echo "ERROR: a file named '-' in $(pwd) would shadow the key on stdin" >&2; exit 1; }
SPARKLE_SIG_LINE=""
if ! SPARKLE_SIG_LINE="$(/usr/bin/security find-generic-password -s https://sparkle-project.org -a ed25519 -w \
    | "$SPARKLE_BIN/sign_update" --ed-key-file - "$ZIP_PATH" 2>/dev/null)"; then
  SPARKLE_SIG_LINE=""
fi
SPARKLE_SIGNATURE="$(echo "$SPARKLE_SIG_LINE" | sed -n 's/.*sparkle:edSignature="\([^"]*\)".*/\1/p')"
SPARKLE_LENGTH="$(echo "$SPARKLE_SIG_LINE" | sed -n 's/.*length="\([^"]*\)".*/\1/p')"
unset SPARKLE_SIG_LINE
[ -n "$SPARKLE_SIGNATURE" ] && [ -n "$SPARKLE_LENGTH" ] || { echo "ERROR: Sparkle signing failed (keychain read or sign_update); output suppressed" >&2; exit 1; }
echo "    edSignature: $SPARKLE_SIGNATURE (length $SPARKLE_LENGTH)"
...
CASK
NOTES_TMP="$CASK_TMP/notes.md"
printf '%s\n' "$RELEASE_NOTES" > "$NOTES_TMP"
bash scripts/release-appcast.sh "$CASK_TMP/tap/appcast.xml" "$VERSION" "$ASSET_URL" "$SPARKLE_LENGTH" "$SPARKLE_SIGNATURE" "$NOTES_TMP"
(cd "$CASK_TMP/tap" && git add Casks/omi.rb appcast.xml && git commit -q -m "omi $VERSION" && git push -q)
rm -rf "$CASK_TMP"
...
# main). It works in a temporary worktree on origin/main so the checkout this
# script runs from is never switched. Failures here warn instead of failing: the
# release itself is already published, and fragments simply stay pending.
consolidate_changelog() {
  local branch="chore/desktop-changelog-$VERSION"
  local wt_root wt rc=0
  git fetch -q origin main || return 1
  wt_root="$(mktemp -d)"
  wt="$wt_root/changelog"
  git worktree add -q -b "$branch" "$wt" origin/main || { rm -rf "$wt_root"; return 1; }
  (
    cd "$wt/desktop/macos" || exit 1
  ...
  git branch -q -d "$branch" 2>/dev/null || echo "    kept unmerged local branch $branch"
  rm -rf "$wt_root"
  return "$rc"
}
