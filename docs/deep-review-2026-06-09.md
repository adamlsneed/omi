...
### AC7. `getActionItems` implemented twice, copies drifted - **fixed (second pass)**
...
- Two top-level functions hit `/v1/action-items` with different query-param shapes (`completed` vs `include_completed`) and different date serialization (UTC-normalized vs raw).
- Second pass: verified against `backend/routers/action_items.py` (read-only) that the server only reads `completed`; `include_completed` was silently ignored. Kept the action_items.dart version, deleted the conversations.dart copy (its callers were already bound to the survivor via `hide getActionItems`, now removed from score_widget.dart).

### AC8. Dead members in CaptureProvider - **fixed (second pass)**
- `capture_provider.dart:394-396` (`_storageStream`/`storageStream`, never assigned), `:384` (`bleBytesStream` getter, no callers), `:445-449` (`setConversationCreating`, commented-out body, no callers) - dead-code, upstream-owned.
- Second pass: all three deleted after re-verifying zero call sites.
...
### AC12. `searchConversationsServer` decodes the response body three times - **fixed (second pass)**
- `app/lib/backend/http/api/conversations.dart:538-540` - perf, low, upstream-owned.
- Second pass: decoded once into a local.

### AC13. People cache JSON re-decoded per transcript event - **fixed (second pass)**
- `capture_provider.dart:165-169` + `preferences.dart:530-537` - perf, low, upstream-owned.
- Second pass: memoized the decoded people list in SharedPreferencesUtil, invalidated on every mutation path (setter, `remove`, `clear`, `init`); `getPersonById` now reads the memo allocation-free.
...
### AU6. Analytics event fired inside build() of summarized-apps sheet - **fixed (second pass)**
- `app/lib/pages/conversation_detail/widgets/summarized_apps_sheet.dart:39-43` - metrics bug, med, upstream-owned
- "Sheet viewed" fires on every notifyListeners while open.
- Second pass: converted to a StatefulWidget; the event fires once per presentation from initState with identical arguments.

### AU7. Conversation detail tab analytics double-fires per tab switch - **fixed (second pass)**
- `app/lib/pages/conversation_detail/page.dart:163-189` - metrics bug, low, upstream-owned. TabController notifies twice per tap.
- Second pass: the listener now ignores notifications while `indexIsChanging`, so taps fire once at settle and swipes still fire once.
...
### AU9. ~12 pages and ~7 widgets are dead code (zero call sites) - **fixed (second pass)**
- `SelectTextScreen`, `CategoryMemoriesPage`, `PrivacyInfoPage`, `DailySummarySettingsPage`, `AboutOmiPage`, `PaypalSetupPage`, `SetupQuestionsPage`, `AboutSdCardSync`, `SdCardTransferProgress`, `RecordingsStoragePermission`, `ConversationCreatedWidget`, `DeviceOnboardingWrapper`; widgets `ConversationAudioPlayerWidget`, `CustomRefreshIndicator`, `GradientButton`, `AnimatedMiniBanner`, `OutOfCreditsWidget`, `ChatAppsDropdownWidget`, `LimitlessSyncCardWidget`, `EmptyAppsWidget` - upstream-owned.
- Second pass: all 20 deleted after re-verifying zero references each (19 whole files, plus the `EmptyAppsWidget` class inside `apps/page.dart`). See the deleted-files ledger below for pull-time guidance. Note: `device_onboarding_page.dart` is now itself orphaned (its only consumer was the deleted wrapper); candidate for a future pass.

### AU10. Large near-duplicate page pairs (upstream copy-paste clusters) - **documented**
- `delete_account.dart` ≈ `cancel_subscription_sheet.dart`; `auto_sync_page.dart` ≈ `sync_page.dart`; `add_app.dart` ≈ `update_app.dart`; `firmware_update.dart` ≈ `omiglass_ota_update.dart`; onboarding pairs; 4x OAuth connect blocks in `task_integrations_page.dart`. Consolidation would be a broad refactor of upstream UI code (onboarding, payments) with real regression risk and maximal merge-surface cost, so it stays documented. When the fork edits one copy, remember to mirror the siblings.

### AU11. Leaked controllers (created, never disposed) - **fixed (second pass)**
...
- Second pass: dispose/cleanup added at all seven sites (each controller verified State-owned first).

### AU12. BuildContext across async gaps (~45 further latent sites) - **fixed (second pass)**
...
- Second pass: guards added at the five enumerated worst sites; the remaining ~40 latent sites stay as-is.
...
### DS9. Verbatim duplicate keychain reader + python-path resolver in Gmail vs Calendar readers - **fixed (second pass)**
- `GmailReaderService.swift:922-944` ≡ `CalendarReaderService.swift:721-743`; identical `pythonPaths` lists. Already diverged once (the DS2 pipe fix existed only in Calendar).
- Second pass: hoisted `keychainPassword(service:)` onto `BrowserKeychainCache` and the python candidate list into a shared `BrowserScriptPython.find()`; both services now use them and their private copies are gone.

### DS10. Dead code in ScreenCaptureService - **fixed (second pass)**
- `testCapturePermission` (:130), `testCaptureCapability` (:136), sync `captureActiveWindow()` (:922), `getActiveWindowID` (:424) - zero call sites.
- Second pass: all four deleted; the live `captureWithScreencapture` fallback is untouched.
...
- `src/services/integrations.rs`, `main.rs:54,173,257` - ~395 lines of unreachable webhook-trigger code; upstream may re-wire it. Kept deliberately: deleting a dormant subsystem upstream still ships maximizes future merge conflicts.
...
### PX4. `mcp/tests/test_server.py` is stale and cannot run - **fixed (second pass)**
- Missing `uid` fixture, outdated signatures, nonexistent enum members; every test errors at collection.
- Second pass: deleted. The healthy `test_search_conversations.py` remains (11/11 pass).

### PX5. Orphaned `web/frontend/sitemap.ts` (wrong location, wrong base URL) + its only dependency - **fixed (second pass)**
...
- Second pass: both files deleted.

### PX6. `PaidAmountDialog` unreferenced; reads a server-only secret from a client component - **fixed (second pass)**
- `src/components/dashboard/paidamount.tsx:39` - would send `Bearer undefined`; if "fixed" by making the key public it would leak an admin credential.
- Second pass: deleted, along with the now-unused `ADMIN_KEY` entry in `envConfig.ts` (`.env` templates untouched).

### PX7. Dead trends/tendencies component tree - **fixed (second pass)**
...
- Second pass: all five deleted; the commented-out JSX mentions in `memories/page.tsx` are inert and left alone.

### PX8. Thumbnail upload pipeline exceeds the server-action body limit for legitimate images - **documented**
- `create-app/page.tsx:126,403` + `next.config.mjs:21` - 10MB-per-file client validation × base64 inflation vs `'10mb'` action body cap; passes validation then fails opaquely. Kept documented: the proper fix is an upload-architecture change in upstream's web app, which the fork does not deploy.

### PX9. React Native SDK ships stale compiled JS shadowing the TS sources under Metro - **fixed (second pass)**
- `sdks/react-native/src/index.js`, `src/types.js` - Metro resolves `.js` first, so consumers load a legacy NativeModules build; `mapCodecToName` is `undefined` at runtime.
- Second pass: both stale files deleted; `package.json` `source`/`react-native` fields now resolve the `.ts` sources (verified by hand, src/ is now 100% TypeScript).
...
### PX11. omiGlass dead OpenAI helpers calling nonexistent endpoints - **fixed (second pass)**
- `sources/modules/openai.ts` `transcribeAudio`/`describeImage` - unimported and unusable as written.
- Second pass: both deleted, plus the consequently-orphaned `imageToBase64`/`blobToBase64` helpers and unused imports. `gptRequest`/`textToSpeech`/`startAudio` remain.
...
| `desktop/Desktop/Sources/GmailReaderService.swift` | DS2, DS9 | `runPython` uses the readabilityHandler drain pattern; shared `BrowserKeychainCache.keychainPassword` + `BrowserScriptPython` live here and replace the per-service copies. |
| `desktop/Desktop/Sources/CalendarReaderService.swift` | DS9 | Private keychain/python helpers deleted in favor of the shared ones in GmailReaderService.swift. |
| `desktop/Desktop/Sources/MainWindow/Pages/ChatLabView.swift` | DS2 | Same drain pattern for `git show`. |
| `desktop/Desktop/Sources/ScreenCaptureService.swift` | DS3, DS10 | Timeout race actually abandons the stalled lookup; four dead members deleted (`testCapturePermission`, `testCaptureCapability`, sync `captureActiveWindow`, `getActiveWindowID`). |
...
| `app/lib/backend/http/api/conversations.dart` | AC7, AC12 | Duplicate `getActionItems` deleted (server only reads `completed`); search decode done once. If upstream consolidates differently, take theirs. |
| `app/lib/backend/preferences.dart` | AC13 | Decoded people list memoized with invalidation on all mutation paths. |
| `app/lib/pages/conversations/widgets/score_widget.dart` | AC7 | Dropped the now-pointless `hide getActionItems` import clause. |
| `app/lib/pages/conversation_detail/widgets/summarized_apps_sheet.dart` | AU6 | StatefulWidget; analytics fires once per presentation. |
| `app/lib/pages/conversation_detail/page.dart` | AU7 | Tab listener ignores `indexIsChanging` notifications. |
| `app/lib/pages/apps/page.dart` | AU9 | Dead `EmptyAppsWidget` class and its three orphaned imports removed. |
| 7 files (AU11): `name_speaker_sheet.dart`, `onboarding/name/name_widget.dart`, `language_selection_dialog.dart`, `add_review_widget.dart`, `app_owner_review_card.dart`, `settings/people.dart`, `processing_conversations/page.dart` | AU11 | Added controller `dispose()`/cleanup only. |
| 5 files (AU12): `conversation_capturing/page.dart`, `memory_graph_page.dart`, `stripe_connect_setup.dart`, `apple_watch_setup_bottom_sheet.dart`, `test_prompts.dart` | AU12 | Added mounted/context.mounted guards only. |
| `web/frontend/src/constants/envConfig.ts` | PX6 | Unused `ADMIN_KEY` entry removed. |
| `omiGlass/sources/modules/openai.ts` | PX11 | Dead `transcribeAudio`/`describeImage` and orphaned helpers removed. |
...
### Deleted upstream files (modify/delete conflicts at pull time)

These upstream-shipped files were deleted as verified-dead code (zero references at deletion time). If a future upstream pull reports a modify/delete conflict on one of them, check whether upstream wired the file into live code: if yes, restore upstream's version; if it is still unreferenced, keep it deleted (`git rm`).

- App pages (AU9): `chat/select_text_screen.dart`, `memories/category_memories_page.dart`, `settings/privacy.dart`, `settings/daily_summary_settings_page.dart`, `settings/about.dart`, `payments/paypal_setup_page.dart`, `onboarding/setup/setup_questions.dart`, `sdcard/about_sdcard_sync.dart`, `sdcard/sdcard_transfer_progress.dart`, `settings/recordings_storage_permission.dart`, `onboarding/conversation_created_widget.dart`, `onboarding/device_onboarding/device_onboarding_wrapper.dart` (all under `app/lib/pages/`)
- App widgets (AU9): `app/lib/widgets/conversation_audio_player_widget.dart`, `app/lib/widgets/custom_refresh_indicator.dart`, `app/lib/widgets/gradient_button.dart`, `app/lib/pages/chat/widgets/animated_mini_banner.dart`, `app/lib/pages/home/widgets/out_of_credits_widget.dart`, `app/lib/pages/home/widgets/chat_apps_dropdown_widget.dart`, `app/lib/pages/capture/widgets/limitless_sync_widget.dart`
- Web (PX5, PX6, PX7): `web/frontend/sitemap.ts`, `web/frontend/src/actions/memories/get-public-memories-prerender.ts`, `web/frontend/src/components/dashboard/paidamount.tsx`, `web/frontend/src/components/memories/tendencies.tsx`, `web/frontend/src/components/memories/tendencies/{trending-banner,trending-item,get-trends}.tsx`, `web/frontend/src/components/trends/trend-item.tsx`
- MCP (PX4): `mcp/tests/test_server.py`
- React Native SDK (PX9): `sdks/react-native/src/index.js`, `sdks/react-native/src/types.js`
...
| --- | --- | --- | --- | --- |
| App core | 13 | 13 | 0 | - |
| App UI + iOS | 12 | 11 | 1 (AU10) | - |
| Desktop Swift | 10 | 10 | 0 | - |
| Desktop Rust + scripts | 6 | 4 | 2 (DR4, DR5) | - |
| Firmware | 5 | 4 | 1 (FW5) | - |
| Backend | 6 | - | - | 6 |
| Peripheral | 11 | 10 | 1 (PX8) | - |
| **Total** | **63** | **52** | **5** | **6** |

The 5 remaining documented items are deliberate keeps: AU10 (broad upstream UI refactor), DR4/DR5 (dormant upstream Rust subsystems whose deletion maximizes merge conflicts), FW5 (untestable DevKit retry-behavior change), PX8 (upload-architecture decision in upstream's web app). Recheck each after every upstream pull.
...
Second pass (deferred-findings remediation, same day):
- App: `flutter analyze` zero new issues vs baseline (53 removed with the deleted files); `app/test.sh` 520/520 passed. Net -3,177 lines.
- Desktop Swift: debug build clean, full test suite passed, clean release build re-run after the DS9/DS10 changes.
- MCP: `py_compile` clean; remaining test suite 11/11 via pytest.
- Web/SDK/omiGlass deletions: exhaustive grep for every deleted symbol and module path (relative and alias imports) found zero live references.
