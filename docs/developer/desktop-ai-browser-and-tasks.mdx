...
`OMI_PYTHON_API_URL` is the primary Python API for auth, transcription, subscription state, conversations, tasks, and most user data. `OMI_DESKTOP_API_URL` points at the hosted desktop backend service and is used for desktop-only proxy routes such as config keys, agent status, and `/v2/chat/completions`.
...
sequenceDiagram
  ...
  Bridge->>Node: pass OMI_MODEL_CREDENTIALS=on_demand
  Node->>Bridge: request model headers per turn
...
`AgentBridge` refuses to start in Omi AI mode if it cannot obtain a Firebase ID token. The token itself never reaches Node: the child environment is a small allowlist with `OMI_MODEL_CREDENTIALS=on_demand`, and Node asks the Swift host for request-scoped model headers over the JSONL channel for each turn.
