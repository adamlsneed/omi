...
class TestV2RecordUsage:
    ...
    def test_record_usage_guarded_by_successful_segments(self):
        ...
        body = self._get_v2_body()
        record_idx = body.find('record_usage,')
        guard_idx = body.find('if successful_segments > 0:')
        assert guard_idx > 0, "successful_segments guard must exist"
        assert guard_idx < record_idx, "record_usage must be guarded by successful_segments > 0"
