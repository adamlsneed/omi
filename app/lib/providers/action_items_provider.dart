...
class ActionItemsProvider extends ChangeNotifier {
  ...
  Future<bool> deleteSelectedItems({BuildContext? context}) async {
    ...
    if (deleted == null || deleted.length != ids.length) {
      ...
      // Clear tombstones for rows the server did not confirm deleted so future
      // refreshes don't filter the re-inserted items. Confirmed rows keep
      // theirs until a refresh proves the server dropped them.
